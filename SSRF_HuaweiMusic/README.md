...
华为音乐 App（版本 12.11.43.300 / 12.11.43.340）中存在 SSRF（Server-Side Request Forgery）漏洞。

`StreamStarter` Activity 被声明为 `exported=true` 且无任何权限保护，通过 `http://` scheme + `audio/*` mimeType 的 intent-filter 暴露给外部应用。该 Activity 接收外部传入的 URI，仅检查 URI 是否以 `http://` 或 `https://` 开头，然后直接将完整的 URI 传递给播放引擎的 `openFileAsync` 方法。该 URI 最终被用于发起 HTTP GET 请求（无任何主机/域名白名单校验、无 SSRF 防护、无内网地址过滤）。
...
### 1. StreamStarter Activity 声明为导出，无权限保护

`com.huawei.music/AndroidManifest.xml`

```xml
<activity android:exported="true" ...>
```

Intent-filter 接受 `http://` scheme + `audio/*` type：
```
Scheme: "http"
Type: "audio/*"
...
### 5. 通过 MediaControllerIml 转发到 MusicUtils

`com.huawei.music/com/huawei/music/playback/impl/MediaControllerIml.java:1489`
```java
public void openFileAsync(String str) {
    MusicUtils.openFileAsync(str);
}
```

### 6. 通过 Binder IPC 调用 MediaPlaybackServiceStub.openFileAsync
...
### 7. Binder 侧的 UID 检查通过（同进程调用），继续处理
...
### 10. 将 URI 传递给 DownloadTask 执行

`com.huawei.music/com/android/mediacenter/playback/player/online/download/task/DownLoadTaskMgr.java:705`
```java
downloadTask.executeOnExecutor(PLAY_POOL, new String[]{downLoadTaskBean.getDownloadPath()});
```

### 11. URL 仅经空格编码处理（无安全校验）
...
### 12. 直接向攻击者控制的 URL 发起 HTTP GET 请求
...
Pb_StreamConnectHelper: getNormalStream startLen:0, totalLen: 0
...
使用内嵌 HTTP 服务器在手机上直接捕获华为音乐发来的请求：
